<?php
    ...
    $pkt = query ("SELECT * FROM paket_layanan WHERE idpaket = $idpaket")[0];
...
<body>
    ...
    <div class="content">
        <div class="isi" style="padding: 10px 10px;">
            <h1>Paket Layanan</h1>
            <h5>Jumlah Paket Layanan = <?= $jmlpaket; ?></h5>
        </div>
        <div class="container kontener">
            <div class="row pb-4 isi">
                <div class="column" style="margin: 15px 15px; width: 70%;"><h2>Ubah Paket Layanan</h2></div>
                <h5 style="margin-left: 20px; width: 50%;">Silahkan Isi Data</h5>
                <form action="" method="post" enctype="multipart/form-data">
                    <input type="hidden" name="idpaket" value="<?= $pkt['idpaket']; ?>">
                    <input type="hidden" name="gambarLama" value="<?= $pkt['gambar_paket']; ?>">
                    <div class="mb-3" style="margin-left: 20px; width: 55%;">
                        <label for="namapkt" class="form-label">Nama Paket</label>
                        <input type="text" class="form-control" id="namapkt" name="namapkt" required value="<?= $pkt['nama_paket']; ?>">
                    </div>
                    <div class="mb-3" style="margin-left: 20px; width: 55%;">
                        <label for="hargapkt" class="form-label">Harga Paket</label>
                        <input type="number" class="form-control" id="hargapkt" name="hargapkt" required value="<?= $pkt['harga_paket']; ?>">
                    </div>
                    <div class="mb-3" style="margin-left: 20px; width: 55%;">
                        <label for="deskripsipkt" class="form-label">Deskripsi Paket</label>
                        <textarea class="form-control" id="deskripsipkt" name="deskripsipkt" rows="3" value="<?= $pkt['deskripsi_paket']; ?>"></textarea>
                    </div>
                    <div class="mb-3" style="margin-left: 20px; width: 55%;">
                        <label for="gambar_paket" class="form-label">Gambar Paket</label><br>
                        <img src="../../assets/<?= $pkt['gambar_paket']; ?>" width="100" ><br><br>
                        <input class="form-control" type="file" id="gambar_paket" name="gambar">
                    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            const hargapktInput = document.getElementById("hargapkt");
            
            hargapktInput.addEventListener("input", function() {
                const value = parseFloat(hargapktInput.value);
                
                if (isNaN(value) || value <= 0) {
                    hargapktInput.setCustomValidity("Harga Paket harus berupa angka positif.");
                } else {
                    hargapktInput.setCustomValidity("");
                }
            });
        });
    </script>
